...
public sealed class EditarCampanhaHandler(
    ILogger<EditarCampanhaHandler> logger,
    IAppDbContext dbContext,
    ICurrentUser currentUser,
    IDateTimeProvider dateTimeProvider,
    IAppLocalizer localizer)
    : IRequestHandler<EditarCampanhaCommand, Result<CampanhaDto>>
{
    public async Task<Result<CampanhaDto>> Handle(EditarCampanhaCommand command, CancellationToken ct)
    {
        ...
        {
            logger.LogWarning("Erro de domínio ao editar campanha {Id}: {Codigo}", command.Id, ex.Codigo);
